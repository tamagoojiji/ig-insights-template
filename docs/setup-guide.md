...
3. **ユースケースの選択を求められた場合**は、以下の優先順位で選んでください：

   | 優先 | 選ぶもの | 理由 |
   |---|---|---|
   | 1️⃣ | **「その他」** または **「カスタマイズ」** | 後の Step 3 で5権限を個別に追加できる。最も確実 |
   | 2️⃣ | 上記が見当たらない場合 → **「Instagramでメッセージとコンテンツを管理」** | `instagram_basic` 等が同梱される。不足分は Step 3 で追加 |

   > 💡 **「アプリの種類」と「ユースケース」は別物です**：アプリの種類＝「ビジネス」、ユースケース＝「その他/カスタマイズ」と覚えてください。両方とも作成中に聞かれることがあります。
...
1. アプリダッシュボード左メニュー「**ユースケースを追加**」をクリック → 下記のような「**さらにユースケースを追加**」ダイアログが開きます

   ![ユースケース追加ダイアログ（フィルターでコンテンツ管理を選択した状態）](images/setup-guide/app-permissions/01-use-case-dialog.png)

   > 💡 **フィルターの読み方**: 左側のフィルターで「おすすめ / すべて / 広告と収益化 / コンテンツ管理 / ビジネスメッセージ / その他」を切り替えて、表示される候補を絞り込めます。

2. ダイアログ内で **「カスタマイズ」**（Customize）を選択 → 「カスタマイズ」ボタンで進む
   - 「カスタマイズ」が見当たらない場合は **「すべて」** フィルターに切り替えるか、画面をスクロールして探してください
